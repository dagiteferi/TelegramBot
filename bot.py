...
async def upload_to_google_drive(file, file_name):
    ...
    # Use a smaller chunk size (256KB) for slow networks.
    media = MediaIoBaseUpload(io.BytesIO(file_data), mimetype=file.mime_type, chunksize=256*1024)
    
    # Run the create call in a thread so it doesn't block the event loop.
    ...
    # Set public permissions asynchronously.
